...
def crawl(settings={}, spider_name="imdb_top_movies", spider_kwargs={}):
    ...
    if is_in_aws():
        ...
        feed_uri = "s3://{os.getenv('S3_BUCKET_NAME')}/%(name)s-{spider_key}.json"
    else:
        feed_uri = "file://feed/%(name)s-{}-%(time)s.json".format(
